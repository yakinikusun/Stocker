...
```
Stocker/
├── .env.example                # 環境変数テンプレート (Supabase URL/Key, 自動消去・在庫変更確定時間)
...
- **二重登録判定（厳格3条件マッチング）**:
  ...
  - 既存在庫判定時（`isExistingMatch`）、画像・タグ・メモ情報は変更不可の固定表示となり、タグはリードオンリーのバッジ、メモは固定テキストとして表示されます。
...
- **一覧の在庫増減の遅延確定**: 在庫一覧 (`StockList`) の `+1/-1` 操作は即時に永続化せず、画面上の数量へ仮反映して保留します。最後の操作から `VITE_PENDING_STOCK_COMMIT_DELAY_MS` で指定した時間が経過した時点で、保留中の複数商品・複数回分の変更をまとめて保存し、商品ごとに合算した変更量を履歴へ記録します。既定値は 2500ms です。数量変更モーダルや在庫追加処理の確定は従来どおり即時です。
...
- **.env による接続固定・自動処理時間設定 ＆ 動作モード切替**:
  - 接続先、0件在庫の自動消去時間、一覧の在庫増減を確定する待ち時間は `.env` (`VITE_SUPABASE_URL`, `VITE_SUPABASE_KEY`, `VITE_ZERO_STOCK_CLEANUP_HOURS`, `VITE_PENDING_STOCK_COMMIT_DELAY_MS`) で設定可能です。`VITE_ZERO_STOCK_CLEANUP_HOURS` のデフォルトは `168` 時間、`VITE_PENDING_STOCK_COMMIT_DELAY_MS` のデフォルトは `2500` ms です。LocalStorage キー接頭辞は `stocker_*` (`stocker_local_products` 等) で統一管理します。「アカウント設定」画面にてワンタップで「Supabase クラウド接続モード」と「LocalStorage オフラインデモモード」を切替可能です。
...
- **手動カメラ切替 ＆ インカメラ/アウトカメラ対応（スマホ・PC共通）**:
  - スマホおよびPC・タブレットにおいて、スキャン画面上の「インカメラに切替 / アウトカメラに切替」ボタンにより、フロント（内カメラ）とリア（外カメラ）を**ワンタップで手動切り替え可能**。
  - インカメラ使用時はユーザーのプレビューを自動で左右反転（ミラー表示 `scale-x-[-1]`）し、自然な視認性を提供（ZXingのバーコード読み取り精度は維持）。
  - 複数カメラデバイスが接続されている環境（外付けWebカメラ等）では、カメラ選択ドロップダウンから特定のカメラハードウェアを直接指定可能。
